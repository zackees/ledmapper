...
function leaderAttachment(anchorX, anchorY, box) {
...
function makePlacement(label, box, displacement, leaderThreshold, flags) {
...
function runLayout(labels, opts, preferredSlot) {
    ...
    const inBounds = (box) => !canvasBounds || (
...
function detectTranslation(prev, labels) {
...
function translatePlacement(p, dx, dy) {
    return {
        ...p,
 ...
 */
export function createLabelLayoutEngine(options = {}) {
    const baseOpts = { ...DEFAULTS, ...options };
    const preferredSlot = new Map();
    let lastInput = null;
    let lastOptsKey = null;
    let lastResult = null;
    ...
    function layout(labels, callOptions = {}) {
...
export function layoutLabels(labels, options = {}) {
