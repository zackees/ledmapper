...
export function getCatalogEntry(id: any) {
 ...
 */
function generateMatrix(cols: any, rows: any, opts: any) {
 ...
 */
function transformPoints(pts: any, rotation: any, flipH: any, flipV: any) {
    if (pts.length === 0) return pts;
    let out = pts.map((p: any) => [p[0], p[1]]);
    const rot = ((rotation || 0) % 360 + 360) % 360;
    const z = (v: any) => (v === 0 ? 0 : v); // normalise -0 → 0
    if (rot === 90) out = out.map(([x, y]: [any, any]) => [z(-y), z(x)]);
    else if (rot === 180) out = out.map(([x, y]: [any, any]) => [z(-x), z(-y)]);
    else if (rot === 270) out = out.map(([x, y]: [any, any]) => [z(y), z(-x)]);
    if (flipH) out = out.map(([x, y]: [any, any]) => [z(-x), y]);
    if (flipV) out = out.map(([x, y]: [any, any]) => [x, z(-y)]);
    return out;
}
 ...
 */
export function generatePanelPoints(entry: any, opts = {}) {
