...
function createDragStateMachine() {
    let drag = null;
    const released = [];   // track releasePointerCapture calls
    const captured = [];   // track setPointerCapture calls
    ...
    function onPointerDown(e, hasPoints = true) {
        if (!hasPoints) return;
        if (e.button === 0) {
    ...
    function onPointerMove(e, hasPoints = true) {
